...
// Customer API (redacted views for staff)
export const customerApi = {
  // List customers (redacted)
  list: (params?: { status?: string; portal_id?: string; search?: string; limit?: number; offset?: number }) => {
    ...
    return fetchApi<{ items: RedactedCustomer[]; total: number }>(
      `${getCpApiUrl()}/cp/customers/redacted${query}`
    );
  },

  // Get single customer (redacted)
  get: (id: string) =>
    fetchApi<RedactedCustomer>(`${getCpApiUrl()}/cp/customers/${id}/redacted`),
};

// Redacted customer type for staff view
export interface RedactedCustomer {
  id: string;
  tenant_id: string;
  display_name: string; // Redacted: "F****n A***n"
