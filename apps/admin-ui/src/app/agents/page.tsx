...
import { Bot, Plus, RefreshCw, Trash2, Power, PowerOff, Settings2, CheckSquare, Square, Layers, Loader2, Edit, AlertCircle } from "lucide-react";
...
export default function AgentsPage() {
  ...
  // Fetch job statuses for agents with current jobs (parallel)
  ...
  const { data: jobStatuses } = useQuery({
    ...
    queryFn: async () => {
      const results = await Promise.all(
        agentJobIds.map(async (jobId) => {
          try {
            const job = await cpApi.getJob(jobId);
