...
export const VALID_TRANSITIONS: Record<JobState, JobState[]> = {
  ...
  // From FORM_FILLING
  FORM_FILLING: ['PROCESSING', 'SLOT_SEARCHING', 'WAITING_HITL', 'FAILED_RETRYABLE', 'FAILED_TERMINAL', 'CANCELLED'],
  
  // From PROCESSING
  PROCESSING: ['SLOT_SEARCHING', 'WAITING_SLOT', 'COMPLETED', 'WAITING_HITL', 'FAILED_RETRYABLE', 'FAILED_TERMINAL', 'CANCELLED'],
  
  // From SLOT_SEARCHING (slot-check can complete with no slot → COMPLETED; booking flow → SLOT_FOUND)
  SLOT_SEARCHING: ['SLOT_FOUND', 'COMPLETED', 'WAITING_HITL', 'FAILED_RETRYABLE', 'FAILED_TERMINAL', 'PAUSED', 'CANCELLED'],
